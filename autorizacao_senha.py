...
def solicita_autorizacao(sope, scli, sprod, smen, samb):
    ...
    if not scli == "":
        hg.conexao_cursor.execute(
            f"UPDATE insopera SET sstatus = 'S',sope = {sope}, scliente = {scli}, "
        ...
        hg.conexao_cursor.execute(
            f"UPDATE insopera SET sstatus = 'S',sope = {sope}, sproduto = {sprod}, "
            f"smensagem = {smen} WHERE scod_op = {mcod_op}"
        )
        hg.conexao_cursor.commit()
        ...
        hg.conexao_cursor.execute(
            f"UPDATE insopera SET sstatus = 'S',sope = {sope}, sambiente = {samb}, "
            f"smensagem = {smen} WHERE scod_op = {mcod_op}"
        )
        hg.conexao_cursor.commit()
    ...
    while True:
        hg.conexao_cursor.execute(
            f"SELECT sstatus,scod_aut FROM insopera WHERE scod_op =  {mcod_op}"
        )
...
def autorizar_senha():
    ...
    mcod_op = mop[0:3]
    hg.conexao_cursor.execute(
        f"SELECT * FROM sacconf WHERE TRIM(modulo) = '{mmodulo.strip()}'"
    )
    arq_conf = hg.conexao_cursor.fetchone()

    hg.conexao_cursor.execute(
        f"SELECT scod_aut,ssenha,snivel FROM insopera WHERE scod_op = '{mcod_op}'"
    )
    arq_usu = hg.conexao_cursor.fetchone()
    maut_oper = mcod_op
    ...
    if m_aut_senha == dcripto(arq_usu[1]):
        ...
        if mmodulo == "LIB_SALDO":
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE QUANTD.SOLICITADA MAIOR QUE O SALDO DO PRODUTO",
                "15",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE TABELA DE CONDICAO PAGAMENTO DIFERENTE",
                "15",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE ESTORNO DE EXPEDICAO DE PRODUTO NO PEDIDO",
                "15",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DA ALTERACAO NO NUMERO NOTA FISCAL",
                "135",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE QUANTD.SOLICITADA MAIOR QUE A QUANTIDADE MAXIMA DA ADM",
                "15",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE OPERADOR COM PEDIDOS PEDENTES",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO P/ALTERACAO NA ENTRADA DE MERCADORIA",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE DESCONTO A MAIOR NO PRECO DE VENDA PRODUTO",
                "15",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE DESCONTO A MAIOR DO QUE O MAXIMO PERMITIDO",
                "15",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE AJUSTE DE SALDO DOS PRODUTOS",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE SALDO A MENOR NA NOTA FISCAL",
                "15",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo, "LIBERACAO P/ALTERAR A SENHA", "1", arq_usu[2], 1, maut_oper
            ):
                mok = "*"
        elif mmodulo == "LIB_AMB":
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE AMBIENTE NAO AUTORIZADO",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DA DATA DE ENTRADA DO SISTEMA",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE DESCONTO NO RECEBIMENTO DO PEDIDO (CAIXA)",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DA QUANTIDADE NA ALTERACAO DO PEDIDO IMPRESSO",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE PEDIDO JA LIBERADOR PELO FINANCEIRO",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE PRAZO MAIOR QUE ESTIPULADO NO PEDIDO",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE CLIENTE DEVEDOR, CHQ.S/FUNDO OU LIMITE ESTOURADO",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DA TAXA DE ACRESCIMO NO PEDIDO",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE PRAZO A MAIOR QUE O PERMITIDO",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE CLIENTE COM *** CHEQUE SEM FUNDOS ***",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE CLIENTE COM *** LIMITE ESTOURADO ***",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE SALDO A RECEBER POR CONTA",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE ORCAMENTO PRECO CUSTO",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE INSCRICAO ESTADUAL",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE VENDEDOR DifERENTE DO VENDEDOR RESP.PELO CLIENTE",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo, "ESTORNO DE PEDIDO EXPEDIDO", "1", arq_usu[2], 1, maut_oper
            ):
                mok = "*"

        elif mmodulo == "LIB_PED":
            if ver_nivel(
                mmodulo, "LIBERAR OU CANCELAR PEDIDOS", "1", arq_usu[2], 1, maut_oper
            ):
                mok = "*"
                # mn_ped = mnum_ped
            ...
            if ver_nivel(
                mmodulo,
                "LIBERAR VALOR MAIOR DO QUE O PEDIDO",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERAR 2a.VIA PEDIDO JA EMITIDO NOTA FISCAL",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo, "EXCLUSAO DE SUB-GRUPO", "1", arq_usu[2], 1, maut_oper
            ):
                mok = "*"
            ...
            if ver_nivel(
                mmodulo,
                "BALANCO ** CORRECAO DE SALDO (SAIDA) **",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DA FORMA DE PAG. NA ALTERCAO DO PEDIDO (AV -> AP)",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERA FORMA DE PAGAMENTO NO RECEBIMENTO DE AVISTA P/APRAZO",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERA PARA Sair O RECEBIMENTO DO PEDIDO",
                "15",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO PARA ALTERAR A CONDICAO DE PAGAMENTO PRE FIXADA DO CLIENTE",
                "15",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO PARA LIMPEZA DO ARQUIVO MOVIMENTO E INTEGRIDADE",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO PARA 2a.VIA DO RECIBO DE ENTREGA",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE ATUALIZACAO DE PRECOS (ENTRADA DE MERCADORIA)",
                "15",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "LIBERACAO DE CANCELAMENTO DE CUPOM FISCAL",
                "15",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "AUTORIZACAO PARA EXCLUSAO DE PRODUTO COM MOVIMENTO",
                "1",
                arq_usu[2],
                1,
                maut_oper,
            ...
            if ver_nivel(
                mmodulo,
                "AUTORIZACAO PARA LIBERAR JUROS E MULTA A RECEBER",
                "1",
                arq_usu[2],
                1,
                maut_oper,
...
def aut_sen(mensagem, mdl, mnum_cli, mnum_prod, mamb, mnum_pedido):
    ...
    tela.setWindowTitle(
        f"MODULO DE LIBERACAO P/SENHA <<Modulo: {mdl} >>         {hg.SISTEMA}  Versao: {hg.VERSAO}"
    )
    hg.conexao_cursor.execute(f"SELECT * FROM sacconf WHERE modulo = '{mdl}'")
