...
def conexao_banco():
    ...
    hg.conexao_cursor.execute(
        f"SELECT * FROM sacindiv where terminal = '{hg.nome_computador}'"
    )
...
def ver_compras(mcod):
    hg.conexao_cursor.execute(
        f"SELECT SUM(valor) FROM sacdupr WHERE fornec = {mcod} AND datpag IS NULL"
    )
...
def ver_nivel(mmodulo, mdescri, mnivel, mconf_nivel, mamb, mopera):
    ...
    if arq_conf is not None:
        ...
        if not presente1 and not presente2:
            if mamb is None:
                print(
                    f"{mdescri}  - ACESSO NAO AUTORIZADO PARA ESTE AMBIENTE - NIVEL: {mconf_nivel}"
                )
                # return False
                aut_sen(
                    f"{mdescri} - Senha de Liberacao do Ambiente:",
                    "LIB_AMB",
                    "",
                    "",
                    "",
...
def gerar_numero_pedido():
    ...
    if arq_noped[0] == 0:
        hg.conexao_cursor.execute(
            f"INSERT INTO sacnoped (numero,SR_DELETED) VALUES ('000000',' ')"
        )
        hg.conexao_bd.commit()
    ...
    hg.conexao_cursor.execute(
        f"update sacnoped set numero = '{mnum_pedido}' where sr_recno = 1"
    )
    hg.conexao_bd.commit()
    return mnum_pedido
...
if __name__ == "__main__":
    ...
    mprg = "SAC140"
    resposta = ver_nivel(
        mprg, "INCLUSAO DE forNECEDOR/CONTA APAGAR", "15", nivel_acess, " ", "  "
    )
    if not resposta:
        print("ok")
