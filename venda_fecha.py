    ...
    if m_tipo_pag == "2":
        ...
        if mqtd_dias >= 2:
            if m_tipo_pag == "3":
                ...
                tela.ds_dia2.setFocus()
                tela.ds_dia2.selectAll()
        if mqtd_dias >= 3:
            if m_tipo_pag == "3":
                tela.ds_dia3.setValue(float(90))
                tela.ds_dia3.setEnabled(True)
                tela.ds_dia3.setFocus()
                tela.ds_dia3.selectAll()
        if mqtd_dias >= 4:
            if m_tipo_pag == "3":
...
def fechar_pedido(m_informa_pedido):
    ...
    # ic(VariavelP.numero_pedido)
    # hg.conexao_cursor.execute(f"SELECT pcod_cli FROM sacped_s WHERE pnum_ped = '{info_inicial_pedido[0]}'")
    ...
    for i in range(tela.cb_forma_pg.count()):
        item_text = tela.cb_forma_pg.itemText(i)
        if str(info_inicial_pedido[5]).strip() in item_text:
